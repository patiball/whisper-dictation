...
class DeviceManager:
    ...
    def __init__(self, enable_logging: bool = True):
        ...
        self.operation_history: Dict[Tuple[str, str], List[bool]] = (
            {}
        )  # (device, operation) -> [success_history]
        ...
        self.logger.info(
            f"DeviceManager initialized. Preferred order: {self.preferred_devices}"
        )
    ...
    def _initialize_device_capabilities(self):
        ...
        for device in self.preferred_devices:
            ...
            self.logger.info(
                f"Device {device} - Basic: {'OK' if basic_capability.available else 'FAIL'}, "
                f"Model: {'OK' if model_capability.available else 'FAIL'}"
            )
    ...
    def _test_model_loading_capability(self, device: str) -> DeviceCapability:
        ...
        try:
            ...
            z = torch.nn.functional.linear(
                y.transpose(1, 2), torch.randn(512, 256, device=device)
            )
            _ = z.sum()
            ...
            error_msg = str(e)[:100] + "..." if len(str(e)) > 100 else str(e)
            self.logger.warning(
                f"Model-like operations failed on {device}: {error_msg}"
            )
            return DeviceCapability(device, False, True, error=error_msg)

    def get_device_for_operation(
        self, operation: OperationType, model_size: Optional[str] = None
    ) -> str:
        """
        ...
        for device in self.preferred_devices:
            capability_key = (
                f"{device}_{operation_key}"
                if operation != OperationType.BASIC_TENSOR
                else f"{device}_basic"
            )

            # Check if device has required capability
            if (
                capability_key in self.capabilities
                and self.capabilities[capability_key].available
            ):
                # Check recent success rate
                history_key = (device, operation_key)
                if history_key in self.operation_history:
                    recent_successes = self.operation_history[history_key][
                        -5:
                    ]  # Last 5 attempts
                    ...
                    if success_rate > 0.8:
                        self.logger.debug(
                            f"Selected {device} for {operation_key} (success rate: {success_rate:.2f})"
                        )
                        return device
                else:
                    # No history yet, try this device
                    self.logger.debug(
                        f"Selected {device} for {operation_key} (no history, trying)"
                    )
                    return device
    ...
    def handle_device_error(
        self, error: Exception, operation: OperationType, current_device: str
    ) -> str:
        """
        ...
        # Log the error
        self.logger.warning(
            f"Device error on {current_device} for {operation_key}: {error_str[:100]}..."
        )
        ...
        if "SparseMPS" in error_str or "aten::empty.memory_format" in error_str:
            ...
            self.logger.error(
                f"Known MPS compatibility issue detected. Disabling {current_device} for {operation_key}"
            )
        ...
        for device in remaining_devices:
            ...
            if (
                ...
                self.logger.info(
                    f"Falling back from {current_device} to {device} for {operation_key}"
                )
                return device

        # Ultimate fallback
        self.logger.info(
            f"Ultimate fallback to {self.fallback_device} for {operation_key}"
        )
        return self.fallback_device
    ...
    def register_operation_success(self, device: str, operation: OperationType):
        ...
        if len(self.operation_history[history_key]) > 10:
            self.operation_history[history_key] = self.operation_history[history_key][
                -10:
            ]
