...
spec = importlib.util.spec_from_file_location(
    "whisper_optimized", "whisper-dictation-optimized.py"
)
...
class TestWhisperPerformance:
    ...
    def test_english_language_detection(self):
        ...
        ), f"Expected English detection, got: {detected_language}"
    ...
    def test_polish_language_detection(self):
        ...
        ), f"Expected Polish detection, got: {detected_language}"
    ...
    def test_language_cache_effectiveness(self):
        ...
        assert (
            transcriber.last_detected_language is not None
        ), "Language should be cached"
        assert (
            second_time < first_time * 1.2
        ), "Second transcription shouldn't be much slower"
    ...
    def test_model_loading_performance(self):
        ...
        if "english" in self.test_files:
            ...
            assert (
                first_transcription_time < 10.0
            ), f"First transcription too slow: {first_transcription_time:.2f}s"
            print(
                f"Model loading: {loading_time:.2f}s, First transcription: {first_transcription_time:.2f}s"
            )
