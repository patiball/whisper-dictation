...
def test_language_detection(audio_file, model_path):
    ...
    try:
        result = subprocess.run(
            transcribe_auto_cmd, capture_output=True, text=True, timeout=30
        )
        print(f"Return code: {result.returncode}")
    ...
    try:
        result = subprocess.run(
            transcribe_pl_cmd, capture_output=True, text=True, timeout=30
        )
        print(f"Return code: {result.returncode}")
    ...
    try:
        result = subprocess.run(
            transcribe_en_cmd, capture_output=True, text=True, timeout=30
        )
        print(f"Return code: {result.returncode}")
