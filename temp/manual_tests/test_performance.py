...
def test_transcription_performance(audio_file, model_path, language=None, expected_language=None):
    """
    ...
    if expected_language and detected_language:
        ...
        print(f"Language detection: {'✓' if language_correct else '✗'} (expected: {expected_language})")
...
def main():
    parser = argparse.ArgumentParser(description='Test whisper transcription performance')
    ...
    if args.audio_files:
        ...
        if not audio_files:
            print("No test audio files found. Please provide --audio-files or create test_*.wav files")
            return
    ...
    for audio_file in audio_files:
        ...
        if expected_language:
            ...
            result_explicit = test_transcription_performance(
                audio_file, model_path, language=expected_language, expected_language=expected_language
            )
            if result_explicit:
    ...
    for result in all_results:
        print(f"{result['audio_file']} ({result['test_type']}): "
    ...
    if all_results:
        ...
        correct_detections = sum(1 for r in all_results if r.get('language_correct', True))
