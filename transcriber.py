...
class SpeechTranscriber:
    ...
    def __init__(self, model_size="base", device=None, allowed_languages=None):
        ...
        if not os.path.exists(model_path):
            ...
            print(
                f"This will download ~{self._get_model_size(model_size)} from internet..."
            )
            user_input = input(f"Download {model_size} model? (y/N): ")
            if user_input.lower() != "y":
                raise FileNotFoundError(
                    f"Model {model_size} not available locally and download refused"
                )
        ...
        try:
            ...
            self.device_manager.base_manager.register_operation_success(
                self.device, OperationType.MODEL_LOADING
            )
            ...
            if self.device_manager.base_manager.should_retry_with_fallback(e):
                fallback_device, user_message = (
                    self.device_manager.handle_device_error_enhanced(
    ...
    def transcribe(self, audio_file_path, language=None):
        ...
        if language:
            ...
            self.model_state = (
                f"{self.model_size}_{self.device}_{language}_{time.time()}"
            )

        # Get optimal device for transcription (may differ from model loading device)
        transcription_device = self.device_manager.get_device_for_operation(
        ...
        try:
            ...
            self.device_manager.base_manager.register_operation_success(
                transcription_device, OperationType.TRANSCRIPTION
            )
        except Exception as e:
            if self.device_manager.base_manager.should_retry_with_fallback(e):
                fallback_device, user_message = (
                ...
                fallback_options = self.device_manager.get_optimized_settings(
                    fallback_device, self.model_size
                )
                if language:
                ...
                self.device_manager.base_manager.register_operation_success(
                    fallback_device, OperationType.TRANSCRIPTION
                )
    ...
    def transcribe_audio_data(self, audio_data):
        ...
        try:
            ...
            if self.device_manager.base_manager.should_retry_with_fallback(e):
                ...
                print(
                    f"✅ Transkrypcja audio zakończona pomyślnie na {fallback_device}"
                )
            else:
                raise e
