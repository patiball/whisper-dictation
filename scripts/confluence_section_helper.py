...
class ConfluenceSectionHelper:
    ...
    def update_section(self, page_id: str, page_title: str, 
                      section_number: int, new_content: str) -> dict:
        """
        Update a specific section by number (1-based).
        Only this section is modified, others remain unchanged.
        """
        # Get page data
        data = self._curl_get(f"/rest/api/content/{page_id}?expand=body.storage,version")
        ...
        if section_number < 1 or section_number > len(sections):
            raise ValueError(f"Invalid section number. Page has {len(sections)} sections.")
        ...
        for i, (t, c, s, e) in enumerate(sections):
            if i == section_idx:
                heading_end = s + len(re.search(r'<h[12][^>]*>.*?</h[12]>', content[s:]).group(0))
                break
    ...
    def update_section_by_title(self, page_id: str, page_title: str,
                               section_title: str, new_content: str) -> dict:
        """
    ...
    def append_to_section(self, page_id: str, page_title: str,
                         section_number: int, content_to_append: str) -> dict:
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  Update section: confluence_section_helper.py update <page_id> <page_title> <section_num> <new_content>")
        print("  Update by title: confluence_section_helper.py update-title <page_id> <page_title> <section_title> <new_content>")
        sys.exit(1)
