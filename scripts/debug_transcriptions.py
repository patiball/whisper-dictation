...
def main():
    ...
    for audio_file in sorted(audio_files):
        ...
        try:
            ...
            print(f"📊 Word Count: Expected {len(expected_words)}, Got {len(transcribed_words)}")
            
            if expected_words and transcribed_words:
                first_match = expected_words[0] in transcribed_words[0:2] if transcribed_words else False
                last_match = expected_words[-1].replace('.', '').replace(',', '') in ' '.join(transcribed_words[-2:]) if transcribed_words else False
                
                print(f"🎯 First word match: {first_match} ('{expected_words[0]}' vs '{transcribed_words[0] if transcribed_words else 'NONE'}')")
                print(f"🎯 Last word match: {last_match} ('{expected_words[-1]}' vs '{transcribed_words[-1] if transcribed_words else 'NONE'}')")
