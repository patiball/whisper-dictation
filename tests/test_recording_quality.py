...
class TestRecordingQuality:
    ...
    def test_recording_start_not_clipped(self, test_audio_dir, sample_texts, text_similarity_checker):
        """
        ...
        if len(immediate_start_files) == 0:
            pytest.skip("No immediate_start test files found. Run record_test_samples.py first")
        ...
        try:
            ...
            first_words = transcribed_words[:3] if len(transcribed_words) >= 3 else transcribed_words
            first_word_found = any(expected_first_word in word for word in first_words)
            
            assert first_word_found, \
    ...
    def test_audio_signal_starts_immediately(self, test_audio_dir):
        ...
        try:
            ...
            max_start_delay = 1.5  # 1.5s maximum - more realistic for real-world conditions
            assert signal_start_time <= max_start_delay, \
            ...
            assert peak_rms > silence_threshold, \
    ...
    def test_recording_start_delay_measurement(self):
        ...
        try:
            ...
            assert avg_delay <= 0.1, f"Average start delay {avg_delay*1000:.1f}ms exceeds 100ms"
            assert max_delay <= 0.2, f"Maximum start delay {max_delay*1000:.1f}ms exceeds 200ms"
    ...
    def test_end_to_end_recording_fidelity(self, test_audio_dir, sample_texts):
        ...
        try:
            ...
            meaningful_words = [word for word in transcribed_words if len(word) > 2 and not word.isdigit()]
            assert len(meaningful_words) >= 2, \
                f"Too few meaningful words in transcription: {meaningful_words}"
            
            print(f"✅ End-to-end test passed: {len(transcribed_words)} words transcribed")
    ...
    def test_microphone_input_levels(self, test_audio_dir):
        ...
        try:
            ...
            for audio_file in audio_files[:3]:  # Test first 3 files
                ...
                max_expected_amplitude = int(32767 * 0.95)  # 95% of max (allowing near clipping)
                
                assert max_amplitude >= min_expected_amplitude, \
                    f"Audio file {audio_file.name} signal too weak: {max_amplitude} (min: {min_expected_amplitude})"
                
                assert max_amplitude <= max_expected_amplitude, \
                    f"Audio file {audio_file.name} signal clipping: {max_amplitude} (max: {max_expected_amplitude})"
                
                print(f"✅ {audio_file.name}: {max_amplitude}/32767 ({max_amplitude/32767:.1%})")
    ...
    def test_audio_quality_metrics(self, test_audio_dir):
        ...
        for audio_file in audio_files:
            try:
                ...
                snr_estimate = 20 * np.log10(rms / (np.std(y) * 0.1))  # Rough SNR estimate
                ...
                if clipping_ratio > 0.01:  # 1% clipping threshold
                    failures.append(f"{audio_file.name}: Clipping detected ({clipping_ratio:.2%})")
                
                if silence_ratio > 0.8:  # 80% silence threshold
                    failures.append(f"{audio_file.name}: Too much silence ({silence_ratio:.2%})")
