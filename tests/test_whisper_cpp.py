...
pytestmark = [
    skip_if_no_whisper_cpp,
    skip_if_env_var_set,
    pytest.mark.whisper_cpp,
    pytest.mark.unit,
    pytest.mark.slow,
]
...
def whisper_model_path() -> str:
    ...
    if not model_path or not Path(model_path).exists():
        pytest.skip(
            f"Model not found at path specified by WHISPER_CLI_MODEL: {model_path}"
        )
    return model_path
...
def run_whisper_cli(
    ...
    *args,
    ...
    capture_stdout_for_parsing: bool = False,
...
def test_timeout_handling(
    whisper_cli_binary, whisper_model_path, english_audio_file, tmp_path: Path
):
    """
    ...
    result_timeout = run_whisper_cli(
        whisper_cli_binary,
        whisper_model_path,
        english_audio_file,
        output_dir=tmp_path,
    ...
    assert isinstance(
        result_timeout, subprocess.TimeoutExpired
    ...
    result_success = run_whisper_cli(
        whisper_cli_binary,
        whisper_model_path,
        english_audio_file,
        "-otxt",
    ...
    assert not isinstance(
        result_success, subprocess.TimeoutExpired
    ...
    ), f"whisper-cli failed with exit code {result_success.returncode}\nStderr: {result_success.stderr}"
...
def test_stderr_error_logging(whisper_cli_binary, whisper_model_path, tmp_path: Path):
    ...
    result = run_whisper_cli(
        whisper_cli_binary, invalid_model_path, "/dev/null", output_dir=tmp_path
    )

    # Oczekujemy, że proces zakończy się błędem (non-zero return code)
    assert (
        result.returncode != 0
    ), f"Expected non-zero return code for invalid model path, but got {result.returncode}"

    # Oczekujemy, że stderr będzie zawierał komunikat o błędzie
    assert (
        "failed to open file" in result.stderr.lower()
...
def test_transcription_not_translation_on_polish_audio(
    whisper_cli_binary, whisper_model_path, polish_audio_file, tmp_path: Path
):
    """
    ...
    assert is_polish(
        transcribed_text
    ), f"Text seems to be translated to English, not transcribed in Polish.\nOutput: '{transcribed_text}'"
...
def test_audio_cutting_regression(
    whisper_cli_binary, whisper_model_path, english_audio_file, tmp_path: Path
):
    """
    ...
    assert (
        expected_start_anchor in transcribed_text
    ), f"Start anchor '{expected_start_anchor}' not found in transcription."
    assert (
        expected_end_anchor in transcribed_text
    ), f"End anchor '{expected_end_anchor}' not found in transcription."
...
def test_language_auto_detection(
    ...
    result = run_whisper_cli(
        ...
        "--detect-language",
        output_dir=tmp_path,
        capture_stdout_for_parsing=True,
    )
...
def test_language_detection_with_confidence(
    whisper_cli_binary, whisper_model_path, english_audio_file, tmp_path: Path
):
    """
    ...
    if len(output_lines) < 2:
        pytest.fail(
            f"Unexpected output format for language detection with confidence: {result.stdout}"
        )
    ...
    ), f"Expected confidence >= {min_confidence}, but got {detected_confidence}"


def test_language_detection_polish_with_confidence(
    whisper_cli_binary, whisper_model_path, polish_audio_file, tmp_path: Path
):
    """
    ...
    print(
        f"Detected Polish language: {detected_language}, Confidence: {detected_confidence}"
    )
...
def test_whisper_cli_internal_timeout(
    whisper_cli_binary, whisper_model_path, english_audio_file, tmp_path: Path
):
    """
    ...
    cmd = [
        ...
        "-otxt",
        "-of",
        str(tmp_path / Path(english_audio_file).stem),
    ]
    ...
    assert (
        result.returncode != 0
    ), f"Expected non-zero return code for timeout, but got {result.returncode}"
    assert (
        "timeout" in result.stderr.lower()
        or "aborted" in result.stderr.lower()
        or "error" in result.stderr.lower()
...
def test_language_auto_detection_polish(
    whisper_cli_binary, whisper_model_path, polish_audio_file, tmp_path: Path
):
    """
