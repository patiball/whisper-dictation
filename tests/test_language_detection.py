...
# Mark all tests as unit tests (slow + manual due to model variability and audio)
pytestmark = [pytest.mark.unit, pytest.mark.slow, pytest.mark.manual]
...
class TestLanguageDetection:
    def test_language_detection_with_exact_text(
        ...
        assert (
            len(audio_files) > 0
        ), "No test audio files found. Run record_test_samples.py first"
        ...
        for audio_file in audio_files:
            ...
            try:
                ...
                transcriber = SpeechTranscriber(
                    model_size="base", allowed_languages=["en", "pl"]
                )
                ...
                if result.language != expected["language"]:
                    failures.append(
                        f"{audio_file.name}: Expected language {expected['language']}, got {result.language}"
                    )
                ...
                expected_start = (
                    expected_norm.split()[0] if expected_norm.split() else ""
                )
                transcribed_words = transcribed_norm.split()

                if not transcribed_words or expected_start not in " ".join(
                    transcribed_words[0:3]
                ):
                    # More flexible: check if start word appears in first 3 words
                    failures.append(
                        f"{audio_file.name}: First word '{expected_start}' not found in start: {transcribed_words[:3]}"
                    )

                # Test 3: Overall text similarity with normalization (more important than exact ending)
                similarity_score = text_similarity_checker(
                    result.text, expected["expected_text"], threshold=0.5
                )
                if not similarity_score:
                    failures.append(
                        f"{audio_file.name}: Text similarity too low. Expected: '{expected['expected_text']}', Got: '{result.text}'"
                    )
    ...
    def test_model_unload_load_on_language_switch(self, test_audio_dir):
        ...
        try:
            ...
            assert (
                initial_model_state != switched_model_state
            ), "Model state should change when switching languages"
            assert (
                polish_result.language == "pl"
            ), f"Polish detection failed: got {polish_result.language}"
            assert (
                english_result.language == "en"
            ), f"English detection failed: got {english_result.language}"

        except Exception as e:
            pytest.fail(f"Model switching test failed: {e}")

    def test_language_detection_accuracy_metrics(
        ...
        for audio_file in audio_files:
            ...
            if "polish" in audio_file.name:
                ...
                expected_lang = (
                    "pl"  # Mixed content often detected as Polish due to Polish words
                )
