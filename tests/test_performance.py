...
class TestPerformance:
    ...
    def test_transcription_performance_speed_ratio(self, test_audio_dir, performance_thresholds):
        """
        ...
        for audio_file in audio_files:
            try:
                ...
                if speed_ratio > performance_thresholds['max_speed_ratio']:
                    failures.append(f"{audio_file.name}: Speed ratio {speed_ratio:.2f}x exceeds target {performance_thresholds['max_speed_ratio']}x")
                
                if speed_ratio > performance_thresholds['critical_speed_ratio']:
                    failures.append(f"{audio_file.name}: CRITICAL - Speed ratio {speed_ratio:.2f}x exceeds {performance_thresholds['critical_speed_ratio']}x")
    ...
    def test_gpu_vs_cpu_acceleration(self, test_audio_dir):
        ...
        try:
            ...
            assert time_gpu < time_cpu * (1 + tolerance), f"GPU ({time_gpu:.2f}s) significantly slower than CPU ({time_cpu:.2f}s) beyond {tolerance*100}% tolerance"
        ...
        for model_size in model_sizes:
            try:
                ...
                if loading_time > max_time:
                    pytest.fail(f"Model {model_size} loading too slow: {loading_time:.2f}s > {max_time}s")
    ...
    def test_memory_usage_during_transcription(self, test_audio_dir):
        ...
        try:
            ...
            assert max_memory < max_total_memory, \
                f"Peak memory {max_memory:.1f}MB exceeds {max_total_memory}MB limit"
                
        except Exception as e:
            pytest.fail(f"Memory usage test failed: {e}")

    def test_batch_transcription_performance(self, test_audio_dir, performance_thresholds):
        """
        ...
        try:
            ...
            assert average_speed_ratio <= performance_thresholds['max_speed_ratio'], \
            ...
            expected_batch_time = total_transcription_time + 5.0  # 5s overhead allowance
            assert batch_total_time <= expected_batch_time * 1.2, \
