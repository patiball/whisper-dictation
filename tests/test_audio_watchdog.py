...
class TestHeartbeatTracking:
    ...
    def test_heartbeat_thread_safety(self, heartbeat_tracker, thread_manager):
        ...
        threads = [
            thread_manager(target=concurrent_updates, args=(i,)) for i in range(5)
        ]
...
class TestStallDetection:
    ...
    def test_stall_detection_timeout_values(self, heartbeat_tracker):
        ...
        assert (
            is_stream_stalled(current_time, 0.001) is True
        )  # Now current time is > 0.001s old
        assert (
            is_stream_stalled(current_time, 1.0) is False
        )  # 1 second timeout should not trigger
    ...
    def test_stall_detection_with_grace_period(self, heartbeat_tracker):
        ...
        def is_stream_stalled_with_grace(
            last_heartbeat, timeout_seconds=5.0, grace_period=1.0
        ):
        ...
        time.sleep(0.1)
        assert (
            is_stream_stalled_with_grace(heartbeat_tracker["get_last"](), 0.05) is False
        )
...
class TestThreadSafety:
    ...
    def test_global_variable_access(self, thread_manager):
        ...
        threads = [
            thread_manager(target=increment_counter, args=(i,)) for i in range(5)
        ]
    ...
    def test_race_condition_prevention(self, thread_manager):
        ...
        def append_data(thread_id, value):
            try:
                ...
                shared_resource["errors"].append(
                    f"Race condition in thread {thread_id}: {e}"
                )
