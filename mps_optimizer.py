...
class MPSErrorHandler:
    ...
    ERROR_PATTERNS = {
        ...
        MPSErrorType.MEMORY_FORMAT: ["aten::empty.memory_format", "memory_format"],
    ...
    def categorize_error(self, error: Exception) -> MPSErrorType:
        ...
        for error_type, patterns in self.ERROR_PATTERNS.items():
            if any(pattern in error_str for pattern in patterns):
                ...
                self.logger.debug(
                    f"Categorized error as {error_type.value}: {error_str[:50]}..."
                )
                return error_type
    ...
    def get_user_friendly_message(self, error: Exception) -> str:
        ...
        messages = {
            MPSErrorType.SPARSE_BACKEND: "Wykryto problem kompatybilności z GPU M1. Przełączam na CPU dla stabilności.",
            MPSErrorType.MEMORY_FORMAT: "Problem z formatem pamięci GPU M1. Używam CPU jako alternatywy.",
            MPSErrorType.OUT_OF_MEMORY: "Brak pamięci GPU. Przełączam na CPU.",
            MPSErrorType.UNSUPPORTED_OP: "Operacja nieobsługiwana przez GPU M1. Używam CPU.",
            MPSErrorType.UNKNOWN: "Nieznany problem z GPU M1. Przełączam na CPU dla bezpieczeństwa.",
        }
...
class MPSOptimizer:
    ...
    def get_optimal_whisper_settings(
        self, device: str, model_size: str
    ) -> Dict[str, Any]:
            ...
            settings.update(
                {
                    ...
                    "beam_size": (
                        5 if model_size in ["tiny", "base"] else 1
                    ),  # Adaptive beam size
...
class EnhancedDeviceManager:
    ...
    def get_device_for_operation(
        self, operation, model_size: Optional[str] = None
    ) -> str:
        """Enhanced device selection with error history"""
        return self.base_manager.get_device_for_operation(operation, model_size)

    def handle_device_error_enhanced(
        self, error: Exception, operation, current_device: str
    ) -> tuple[str, str]:
        ...
        fallback_device = self.base_manager.handle_device_error(
            error, operation, current_device
        )

        # Log enhanced error info
        error_type = self.error_handler.categorize_error(error)
        self.logger.info(
            f"Enhanced error handling: {error_type.value} -> {fallback_device}"
        )
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        for device in self.base_manager.preferred_devices:
            base_status["memory_info"][device] = self.optimizer.get_memory_usage_info(
                device
            )
