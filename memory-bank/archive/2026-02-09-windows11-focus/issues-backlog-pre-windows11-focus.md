...
### Issue 0: Long-Running Process Memory Leak - Crash After Hours
**Status**: NEW - High Priority  
**Priority**: 🔴 CRITICAL  
**Description**: Application crashes with `zsh: abort` signal after several hours of continuous operation
- **Symptoms**: Process runs successfully for 2-3 hours, then suddenly aborts
- **Last seen**: 2025-11-24 after ~3 hours of continuous use with large model
- **Error**: `zsh: abort      poetry run python whisper-dictation-fast.py -m large --k_double_cmd`
- **Model affected**: Tested with `large` model, likely affects all models

**Root Cause Analysis**:
1. **Memory leak in PyAudio** (line 174-193 in `whisper-dictation-fast.py`)
   - `pyaudio.PyAudio()` instance created per recording but cleanup is incomplete
   - Stream termination may not fully release resources
   - Accumulates memory with each recording cycle

2. **Subprocess resource leak**
   - Each transcription creates new `whisper-cli` subprocess
   - GPU/VRAM not properly released after process completion
   - Applies to all models but severity increases with `large` model size

3. **Threading resource leak**
   - Recording threads created but may not properly join
   - Timer threads (line 185, 300) accumulate without explicit cleanup
   - No daemon thread configuration

4. **GPU Memory (VRAM) exhaustion**
   - Model `large` uses significant VRAM on Metal GPU
   - Multiple concurrent processes or improper cleanup cause accumulation
   - Results in system abort when VRAM exhausted

**Evidence**:
```
[15:14:09.255] Listening...
zsh: abort      poetry run python whisper-dictation-fast.py -m large --k_double_cmd
```
Process crashed during listening phase after successful transcriptions.

**Files to Modify**:
- `whisper-dictation-fast.py` (lines 157-204) - `Recorder._record_impl()` cleanup
- `whisper-dictation-fast.py` (lines 38-123) - `SpeechTranscriber.transcribe()` subprocess cleanup
- `whisper-dictation-fast.py` (lines 229-307) - `StatusBarApp` timer cleanup
- `recorder.py` (lines 142-172) - `_record_background()` resource management

**Acceptance Criteria**:
- [ ] PyAudio resources properly released after each recording (stream.close() + p.terminate())
- [ ] Subprocess resources cleaned up (explicit garbage collection or context managers)
- [ ] Threading resources properly managed (join threads, daemon mode, or explicit cleanup)
- [ ] GPU memory monitoring shows stable usage over 8+ hour continuous run
- [ ] Application remains stable for minimum 24 hours continuous operation
- [ ] Memory profiling shows no accumulation trend

**Testing**:
- [ ] Run continuous recording session (cycle: record 5s, transcribe, repeat) for 24 hours
- [ ] Monitor system memory and GPU memory throughout
- [ ] Log resource allocation/deallocation events
- [ ] Test with all model sizes (tiny, base, small, medium, large)
- [ ] Verify no zombie processes remain after app termination

**Related Issues**:
- Issue 1: Audio Clipping (may be caused by resource contention)
- Issue 4: Transcription Delay (may be worsened by memory pressure)
...
**Created**: 2025-10-20  
**Last Updated**: 2025-11-24 (Added Critical: Memory Leak Issue #0)
**Session**: Audio Issues Review + Memory Leak Investigation  
**Reporter**: User (Marcin Przybyszewski)  
**Status**: Active - Critical Issue Identified and Documented
